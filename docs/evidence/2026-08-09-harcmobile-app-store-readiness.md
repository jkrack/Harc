...
| iPhone signed build and smoke automation | **Current-candidate diagnostic pass** | After regenerating the project, HarcMobile 0.14.1 (56) passed a signed bounded run on Omega: all 24 hosted app tests and all 6 UI tests, with zero failures or skips. UI coverage includes the native accessibility audit, ordinary microphone start/stop to **Saved locally**, C5 force-quit recovery, and C7 storage exhaustion. CoreDevice then confirmed 0.14.1 (56) installed on the phone. The dirty worktree prevents promotion to sealed release evidence; the 14 spike-harness guard tests passed earlier but were not rerun in this exact 0.14.1 suite. |
...
| Public privacy-policy surface | **Pass in source and focused physical tests** | A resolving public HTTPS policy URL is packaged in the app, displayed under **Privacy & Data**, and copied into the metadata deck. The configuration assertion passed on Omega; recheck after the public commit is pushed. |
| Public support surface | **Local structure ready; contact and publication open, Account Holder** | A stable support-page path, product help, privacy warning, and issue-intake link are prepared locally. The exact public URL currently returns HTTP 404 because the file is not on pushed `main`. Replace the explicit email placeholder with an actually monitored address, commit/push the page, and require HTTP 200 before submission. GitHub Issues alone is not the contact surface. |
...
| Screenshots | **Plan ready; capture open** | Five-image 6.9-inch plan is prepared. Capture from the exact release build. |
| App icon | **Pass in source** | `HarcMobileApp/Assets.xcassets` contains the established Harc artwork as one opaque 1024-by-1024 universal iOS AppIcon source. The executable preflight validates dimensions and absence of alpha; exact-archive asset compilation remains part of the archive gate. |
| Exact clean archive and upload | **Open** | Requires a clean evidence commit, archive validation, distribution signing/profile, archived executable SHA-256 and signature CDHash, upload, and App Store processing result. |
...
| Executable App Store preflight | Added a fail-closed repository/archive verifier covering disk floor, plists/privacy, release settings, icon properties, metadata limits, support URL/contact, and exact archive contents, plus a separate exported-app check for distribution certificate/profile and debugger-disabled entitlements. It pins the exact Harc team/application identifier, requires a future profile expiration, and rejects device-bound or enterprise-wide profiles at the App Store distribution stage. This reflects Xcode's archive-versus-export/upload signing stages. The current repository report passes the final icon and has one owner-input open: the monitored support email. Exact-candidate archive and distribution validation remain unexecuted. |
...
| Public-URL preflight | Added an optional live HTTPS check for the exact privacy-policy and support URLs. Current pushed `main` returns HTTP 200 for the privacy policy and HTTP 404 for the uncommitted support page, correctly leaving support publication open. |
...
1. Replace the monitored support-email placeholder with the Account Holder's
   real support address and publish the support page.
2. Capture the five screenshots from the exact release build.
3. Create one clean evidence commit and run the Release archive preflight.
4. Execute the physical qualification matrix with the declared oldest and
   current non-Pro hardware and preserve reports, hashes, logs, and visible
   outcomes.
5. Complete the secondary-Mac and remaining physical two-network/direct-route/
   visible-revocation evidence plus old-record/account-export expiry evidence.
6. Reconcile the final archive with App Privacy/export answers, upload it, and
