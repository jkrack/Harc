...
**Phase 3 status: complete.** Foundation — `DictationContext` (value type + `promptBlock` markdown rendering with per-field 4000-char truncation) and `SelectionContextReader` (AX selected text / clipboard / frontmost app behind an injectable `Environment` seam; never prompts for Accessibility). Wiring — per-mode `includeSelectedText`/`includeClipboard` toggles (backward-compatible Codable; editor UI with local-privacy footnote), capture at dictation start in `DictationController` (before the HUD shows; skipped entirely for deny-listed frontmost apps), context snapshot on `DictationState` cleared at session end, `ModeTransformPrompt.build(instruction:transcript:contextBlock:)` placing context before the dictated-text fence with a context-hardened system prompt, and the HUD context-capture indicator. A sixth built-in mode, **Answer** (`builtin.answer`, context-on), is the Super-Mode analog: dictate a request, it acts on the selection/clipboard.
